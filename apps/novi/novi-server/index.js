...
const init = (config) => {
  ...
  app.post('/sign', cors(), async (req, res) => {
    ...
    if (K.getSeed()) {
      ...
      res.json({ error: "Please check that Novi is running and you're signed in. Check Novi dashboard at http://localhost:21000", code: 1 })
    }
  })
  ...
  app.listen(port, () => {
    console.log(`Novi listening at http://localhost:${port}`)
  })
}
module.exports = init
