import listActions from 'modules/apps/list/appsListActions';
...
const actions = {
  ...
  doDestroy: (id) => async (dispatch) => {
    try {
      ...
      await AppsService.destroyAll([id]);

      dispatch({
        type: actions.DESTROY_SUCCESS,
      });

      Message.success(i18n('entities.apps.destroy.success'));
  ...
  doDestroyAll: (ids) => async (dispatch) => {
    try {
      ...
      await AppsService.destroyAll(ids);
      ...
      Message.success(
        i18n('entities.apps.destroyAll.success'),
      );
